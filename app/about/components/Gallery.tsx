...
type PhotoProps = {
  src: string;  // Changed to string for public/ images
  meta?: ReactNode;
  filename?: string;
  alt: string;
  width: number;
...
function Photo({
  ...
  const fileName =
    filename ||
    src.split("/").at(-1)?.split(".")[0]?.replace(/-/g, " ").replace(/^\w/, c => c.toUpperCase()) || "Photo";
  ...
  return (
    <motion.div
      ...
      initial={{
        ...
        rotate: (rotate || 0) - 20,
        y: 200 + index * 20,
        x: index === 1 ? -60 : index === 2 ? -30 : index === 3 ? 30 : index === 4 ? 60 : 0,
        opacity: 0,
      }}
      transition={{
        default: {
      ...
      >
        <div className={shared} style={{ backfaceVisibility: "hidden" }}>
          <Image
            ...
            priority
            unoptimized  // Add if images are animated/large
          />
          {children}
        </div>
        <div
  ...
  return (
    <>
      <section className="flex gap-4 h-[268px] relative">
        <Photo
          src="/gallery/florida.jpg"
          meta="2025-01-10"
          alt="Las Olas Riverwalk"
          width={324}
          height={239}
          rotate={-6}
          left={-56}
        ...
        />
        <Photo
          src="/gallery/riverside.jpg"
          meta="2024-04-13"
          alt="Riverside Park, LAX"
          width={230}
          height={250}
          rotate={6.3}
          left={160}
